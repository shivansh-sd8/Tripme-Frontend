...
// Add this line after line 93
const [filterOpen, setFilterOpen] = useState(false);
const [filteredResultsCount, setFilteredResultsCount] = useState(0); // Add this line
 ...
 const applyFilters = async (filters: any) => {
  try {
    ...
    if (data.success) {
      ...
      // If data.data has a listings property, use that
      ...
      if (!Array.isArray(listingsData)) {
        console.warn('⚠️ Expected array but got:', typeof listingsData, listingsData);
        listingsData = [];
      }
...
//       await apiClient.removeFromListingWishlist(stayId);
//       setFavorites(prev => {
...
//       // await apiClient.addToListingWishlist(stayId);
...
// const handleFavorite = async (stayId: string) => {
//   console.log('🔥 HANDLE FAVORITE HIT',wishlists);
...
//     //   prev.map(wl =>
//     //     wl._id === found.wishlistId
//     //       ? { ...wl, items: wl.items.filter((i: any) => i._id !== found.wishlistItemId) }
//     //       : wl
//     //   )
//     // );
//     setWishlists(prev =>
...
//   await apiClient.addToWishlist(wishlist._id, {
//     itemType: 'Property',
//     itemId: stayId
//   });
...
//     prev.map(wl =>
//       wl._id === wishlist._id
//         ? { ...wl, items: [...wl.items, { itemId: stayId }] }
//         : wl
//     )
//   );
// }
  ...
  onResultsCountChange={handleResultsCountChange}
      ...
      <div className="pt-20">
         ...
         {/* <button
          ...
          className="lg:hidden fixed bottom-24 left-1/2 -translate-x-1/2 z-40 bg-gray-900 text-white px-6 py-3 rounded-full font-semibold shadow-lg flex items-center gap-2 hover:bg-gray-800 transition-all"
        >
          {showMobileMap ? (
            <>
